...
// ✅ FIXED: these values match exactly what the backend accepts
// Backend: open | fixed | accepted_risk | false_positive
const STATUS_OPTIONS: { value: Finding['status']; label: string; icon: string }[] = [
  { value: 'open',           label: 'Open',           icon: '○' },
  { value: 'fixed',          label: 'Fixed',          icon: '✓' },
  { value: 'accepted_risk',  label: 'Accepted Risk',  icon: '~' },
  { value: 'false_positive', label: 'False Positive', icon: '✗' },
];

const STATUS_STYLE: Record<string, { color: string; bg: string; border: string }> = {
  open:           { color: 'var(--warn)',      bg: 'var(--warn-dim)',    border: 'rgba(255,170,0,0.3)'   },
  fixed:          { color: 'var(--accent)',    bg: 'var(--accent-dim)', border: 'var(--accent-border)'  },
  accepted_risk:  { color: 'var(--severity-medium)', bg: 'rgba(255,170,0,0.08)', border: 'rgba(255,170,0,0.2)' },
      ...
      >
        ...
        >
          <span style={{ color: 'var(--text-faint)' }}>
            Scan:{' '}
            <span style={{ color: 'var(--text-secondary)' }}>...{finding.scan_id?.slice(-14)}</span>
          </span>
          {finding.scan_info?.scan_date && (
            <span style={{ color: 'var(--text-faint)' }}>
              Scanned:{' '}
              <span style={{ color: 'var(--text-secondary)' }}>{formatDate(finding.scan_info.scan_date, true)}</span>
            </span>
          )}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {STATUS_OPTIONS.map(opt => {
                ...
                const scfg     = STATUS_STYLE[opt.value];
                return (
                  <button
                    key={opt.value}
                    disabled={updating}
                    onClick={() => handleStatusChange(opt.value)}
                    className="px-3 py-1.5 rounded-sm text-xs font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5"
                    style={
                      isActive
                        ? { color: scfg.color, backgroundColor: scfg.bg, border: `1px solid ${scfg.border}`, fontWeight: 700 }
                        : { color: 'var(--text-muted)', backgroundColor: 'transparent', border: '1px solid var(--border-default)' }
                    }
                    onMouseEnter={e => {
                      if (!isActive && !updating) {
                        const el = e.currentTarget as HTMLElement;
                        el.style.color = scfg.color;
                        el.style.backgroundColor = scfg.bg;
                        el.style.borderColor = scfg.border;
                      }
                    }}
                    onMouseLeave={e => {
                      if (!isActive) {
                      ...
                      <span className="flex items-center gap-1.5">
                        <span
                          ...
                          style={{
                            ...
                            borderRightColor: scfg.color,
                            borderBottomColor: scfg.color,
                            borderLeftColor: scfg.color,
                          }}
                        />
                        Saving...
                      </span>
        ...
        >
          <p className="text-xs" style={{ color: 'var(--text-faint)' }}>
            ...
            {finding.updated_at && <span> · Updated: {formatDate(finding.updated_at)}</span>}
          </p>
          <button
            onClick={onClose}
